...
// ===== Missing Helper Functions for Dashboard Analytics =====
function getDayOfWeek(dateStr) {
  if (!dateStr) return '';
...
function calculateMsgStats(text) {
  if (!text) return { charCount: 0, charCountNoSpaces: 0, lineCount: 0, emojiCount: 0, emojiList: [], personalizationCount: 0, personalizationList: [], urlCount: 0 };
  const charCount = text.length;
  const charCountNoSpaces = text.replace(/\s/g, '').length;
  const lineCount = text.split('\n').length;
  
  // Emoji detection regex
  const emojiRegex = /[\u{1F300}-\u{1F9FF}\u{1F600}-\u{1F64F}\u{1F680}-\u{1F6FF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}\u{1F1E6}-\u{1F1FF}\u{1F900}-\u{1F9FF}\u{1F300}-\u{1F5FF}\u{1F900}-\u{1F9FF}\u{1D000}-\u{1F77F}\u{1F000}-\u{1F0FF}]/gu;
  const emojiMatches = text.match(emojiRegex) || [];
  const emojiCount = emojiMatches.length;
  const emojiList = [...new Set(emojiMatches)];
  
  // Personalization variable detection (e.g. [이름], [날짜], ${이름})
  const personalizationRegex = /\[[^\]]+\]/g;
  const personalizationMatches = text.match(personalizationRegex) || [];
  const personalizationCount = personalizationMatches.length;
  const personalizationList = [...new Set(personalizationMatches)];
...
function findMostSimilarCampaign(c, pastCampaigns) {
  if (!pastCampaigns || pastCampaigns.length === 0) return null;
  
  const currentWeekDay = getDayOfWeek(c.sendDate);
  const currentCharCount = c.msgBody ? c.msgBody.length : 0;
  const currentCtaCount = c.ctaLinks ? c.ctaLinks.length : 0;
  ...
  pastCampaigns.forEach(pc => {
    ...
    // 1. Content length difference (normalized)
    const pcCharCount = pc.msgBody ? pc.msgBody.length : 0;
    score += Math.abs(currentCharCount - pcCharCount) * 0.1;
    
    // 2. Weekday matching (if different, add penalty)
    const pcWeekDay = getDayOfWeek(pc.sendDate);
    if (currentWeekDay !== pcWeekDay) {
      score += 50;
    }
    
    // 3. CTA count difference (penalty per button difference)
    const pcCtaCount = pc.ctaLinks ? pc.ctaLinks.length : 0;
    score += Math.abs(currentCtaCount - pcCtaCount) * 20;
    
    // 4. Channel matching
    if (c.channel !== pc.channel) {
      score += 30;
    }
